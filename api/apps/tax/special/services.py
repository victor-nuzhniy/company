"""DB service functionality for tax base apps."""
...
def get_tax_data(
    ...
    query = (
        TaxInvoice.query.with_entities(
            ...
            func.sum(
                (TaxInvoiceProduct.quantity * SaleInvoiceProduct.price),
            ).label(
                "sale_summ",
            ),
            func.sum(
                (TaxInvoiceProduct.quantity * PurchaseInvoiceProduct.price),
            ).label(
                "purchase_summ",
            ),
        )
...
def get_tax_invoice_products_by_tax_invoice_id(tax_invoice_id: int) -> Sequence:
    ...
    return (
        db.session.query(
            ...
            TaxInvoiceProduct.sale_invoice_product_id.label(
    ...
    **kwargs: Any,
    ...
    purchase_invoice_product = (
        PurchaseInvoiceProduct.query.with_entities(
            PurchaseInvoiceProduct.products_left,
        )
        .filter(
            PurchaseInvoiceProduct.id == kwargs.get("purchase_invoice_product_id"),
        )
        .first()
    )
