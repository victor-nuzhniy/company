...
from tests.apps.account.conftest import get_purchase_products, get_tax_products
...
class TestProcessSaleInvoiceRoute:
    ...
    def test_post_route(self, auth_header: Dict, faker: Faker) -> None:
        ...
        for i, product in enumerate(get_tax_products(sale_invoice.id)):
            assert sale_invoice_products[i].id == product.sale_invoice_products_id
            assert (
                purchase_invoice_products[i].id == product.purchase_invoice_products_id
            )
            assert product.quantity == sale_invoice_products[i].quantity
            assert expected_purchase_products[i].products_left == 100
        assert response.status_code == 200
        assert result.get("message") == "Operation successfully performed"

    def test_post_route_sale_invoice_done(self, auth_header) -> None:
        ...
        assert (
            result.get("message")
            == f"Sale_invoice with id {sale_invoice.id} is already done!"
        )
